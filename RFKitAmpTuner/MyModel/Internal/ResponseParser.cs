    ...
    internal class ResponseParser
    {
        ...
        private void ProcessParsedResponse(string key, string value, StatusUpdate update, StatusTracker tracker)
        {
            switch (key)
            {
                ...
                case Constants.KeyFpw:
                    ...
                    // commented out by N3OC and set VFWD in ParsePowerSwr since RFKIT API does not implement tuner forward power and this allows the Tg panel to display forward power instead of vfwd which is not implemented by the RFKIT API
                ...
                case Constants.KeyInd:
                    // Inductor value (hex) - Tg panel has a maximum of 256 (0x100) and any higher numbers
                    // will cause the meter bar to roll over (i.e. 500 will display as 244 - 256+244=500)
                    if (int.TryParse(value, NumberStyles.HexNumber, CultureInfo.InvariantCulture, out int inductors))
                    {
                        // Log the value of inductors - added by N3OC
                        //Logger.LogLudicrous(ModuleName, "Tuner inductors:" + inductors.ToString());
                        // try dividing inductors by 2 to see if it matches the expected range (0-256) for the Tg panel
                        // as expected, it now shows half the value reported by the tuner, which suggests the tuner is sending a value in a 0-512 range instead of 0-256, and dividing by 2 corrects it for display on the Tg panel
                        inductors = inductors / 2;
                ...
                case Constants.KeyCap:
                    // Capacitor value (hex) - Tg panel has a maximum of 256 (0x100) and any higher numbers
                    // will cause the meter bar to roll over (i.e. 500 will display as 244 - 256+244=500)
                    if (int.TryParse(value, NumberStyles.HexNumber, CultureInfo.InvariantCulture, out int capacitors))
                    {
                        // Log the value of capacitors - added by N3OC
                        //Logger.LogLudicrous(ModuleName, "Tuner capacitors:" + capacitors.ToString());
                        // try dividing capacitors by 2 to see if it matches the expected range (0-256) for the Tg panel
                        // as expected, it now shows half the value reported by the tuner, which suggests the tuner is sending a value in a 0-512 range instead of 0-256, and dividing by 2 corrects it for display on the Tg panel
                ...
                case Constants.KeyAnt:
                    if (int.TryParse(value, out int ant))
                        // log the antenna value - added by N3OC
                        // this never seems to run, why?
                        Logger.LogLudicrous(ModuleName, "Tuner antenna:" + ant.ToString());
                    update.Antenna = ant;
                    break;
        ...
        private void ParsePowerSwr(string value, StatusUpdate update)
        {
            ...
            if (parts.Length >= 2)
            {
                ...
                {
                    // added by N3OC to set tuner forward power (VFWD) to the same value as amplifier forward power since the RFKIT API does not implement tuner forward power and this allows the Tg panel to display forward power instead of vfwd which is not implemented by the RFKIT API
                    update.VFWD = power;
