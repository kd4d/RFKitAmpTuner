...
// added using PgTg.Common by N3OC so Logger can be used in this class
using PgTg.Common;
...
namespace RFKitAmpTuner.MyModel.Internal
{
    ...
    internal static class RfkitCommandMapper
    {
            ...
            Action<string, string>? logVerbose = null)
        {
            ...
            //changed to $OS1 for operate and $OS0 for standby by N3OC
            if (t.Equals(Constants.OperateCmd.TrimEnd(';'), StringComparison.Ordinal) || t.Equals("$OS1", StringComparison.Ordinal))
            {
            ...
            if (t.Equals(Constants.StandbyCmd.TrimEnd(';'), StringComparison.Ordinal) || t.Equals("$OS0", StringComparison.Ordinal))
            {
            ...
            if (t.Equals("$FLT", StringComparison.Ordinal))
            {
                using var doc = client.Get(RfkitRestPaths.Data);
                // log result
                Logger.LogLudicrous(ModuleName, $"GET data for $FLT: {RfkitCatFromJson.FltLineFromData(doc.RootElement)}");
            ...
            // not implemented yet by RFKIT API
            ...
            // set antenna command mapping with "$ANT n" syntax, where n is 1, 2, 3 or 4, added by N3OC to support 4 antennas for RFKIT
            ...
            // $ANT polling response mapping
            if (t.Equals("$ANT", StringComparison.Ordinal))
            {
                using var doc = client.Get(RfkitRestPaths.AntennasActive);
                return doc == null ? null : RfkitCatFromJson.AntLineFromAntennas(doc.RootElement);
            }
        ...
        private static void TryPutAntenna(IRfkitRestClient client, string t, Action<string, string>? logVerbose)
        {
            ...
            // > 4 by N3OC to support 4 antennas for RFKIT
            if (!int.TryParse(parts[1], out var n) || n is < 1 or > 4)
                return;
            ...
            var body = n switch
            {
                ...
                3 => """{"type":"INTERNAL","number":3}""",
                // added antenna 4 by N3OC
                4 => """{"type":"INTERNAL","number":4}"""
            };
